...
async function runDocumentAclMigration() {
    try {
        console.log('Running document_acl migration...');
        
        // Read migration file
        const migrationPath = path.join(__dirname, '../mindmap-notion-interface/supabase/migrations/20251008000500_create_document_acl.sql');
            ...
            if (rawError) {
                ...
                if (statement.includes('CREATE TABLE')) {
                    ...
                    const { error: tableError } = await supabase
                        ...
                        .select('*')
                        .eq('table_name', 'document_acl');
                        
                    if (tableError && tableError.code === 'PGRST116') {
                        console.log('Table does not exist, need to create via Supabase Dashboard');
                        console.log('Please manually create the document_acl table using the SQL in the migration file');
                    } else if (!tableError) {
